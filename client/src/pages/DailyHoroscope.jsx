...
export default function DailyHoroscope() {
    ...
    // Check if prediction is an error message (string) or actual data (object)
    const isErrorString = typeof data.prediction === 'string';
    const bot = !isErrorString ? data.prediction?.bot_response : null;
            ...
            <div className="flex flex-col items-center text-center space-y-4">
                ...
                <div className="flex items-center gap-4 text-slate-500 text-xs font-bold uppercase tracking-widest">
                    ...
                    <span className="text-amber-400/80">
                        Global Alignment: {bot?.total_score?.score ?? 0}%
                    </span>
                </div>
            </div>

            {isErrorString ? (
                /* Error Message UI when API calls are out */
                <div className="bg-slate-900/40 border border-red-500/20 p-10 rounded-[2.5rem] flex flex-col items-center text-center space-y-4">
                    <AlertCircle className="text-red-400" size={48} />
                    <h3 className="text-xl font-bold text-white uppercase tracking-wider">Cosmic Connection Interrupted</h3>
                    <p className="text-slate-400 max-w-md">"{data.prediction}"</p>
                    <button 
                        onClick={() => window.location.reload()}
                        className="mt-4 px-6 py-2 bg-amber-500 text-black text-xs font-black uppercase rounded-full hover:bg-amber-400 transition-colors"
                    >
                        Retry Sync
                    </button>
                </div>
                ...
                <>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        ...
                        <StatCard 
                            title="Love" 
                            score={bot?.relationship?.score ?? 0} 
                        ...
                        <div className="relative flex flex-col md:flex-row items-start md:items-center gap-8">
                            ...
                            <div>
                                ...
                                <p className="text-xl md:text-2xl text-slate-200 font-bold leading-tight tracking-tight">
                                    "{bot?.total_score?.split_response ?? "Summary unavailable"}"
                                </p>
                            </div>
                        </div>
                    </div>
            ...
            <div className="space-y-6">
                ...
                <div className="space-y-3">
                    ...
                    <p className="text-sm text-slate-400 leading-relaxed font-medium">
                        {desc || "No specific guidance for this sector today."}
                    </p>
                </div>
            </div>
